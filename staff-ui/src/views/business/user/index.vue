          ...
          >
            <template slot-scope="scope" >
              <el-button
                ...
                v-if="scope.row.superAdministrator !== '集团管理员'"
              >修改</el-button>
              <el-button
                size="mini"
                type="text"
...
import {
  ...
  changeUserStatus,
  deptTreeSelect,
  listHotelUser, getUserHotel, addHotelUser, updateHotelUser
} from "@/api/system/user";
import { getToken } from "@/utils/auth";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import {list, listHotel, listHotelAll, updateHotel} from "@/api/business/hotel";

export default {
  name: "User",
  dicts: ['sys_normal_disable', 'sys_user_sex','super_administrator'],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHotelUser(this.queryParams).then(response => {
          this.userList = response.rows;

          this.userList.forEach(u => {
            u.superAdministrator = u.superAdministrator === 0 ? "集团管理员" : "酒店管理员"
          })
    ...
    handleUpdate(row) {
      ...
      this.getHotels();
      getUserHotel(userId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.userId !== undefined) {
            updateHotelUser(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            if(this.form.hotelId === "" || this.form.hotelId === undefined) {
              this.$message.error("酒店管理员必须设置酒店ID")
              return;
            }
            addHotelUser(this.form).then(response => {
